describe('results spec', () => {
  ...
  it('should allow users to generate a logo', () => {
    cy.completeForm()
    cy.get('.form-button').click()
    .get('.custom-logo-box > .design-header-container > .design-header-background > .design-header').should('have.text', 'Exclusive Feature')
    .get('.logo-text').should('have.text', 'Want a custom generated logo?')
    .intercept('PUT', 'https://ai-project-planner-be-72e73912044c.herokuapp.com/api/v1/users/1/projects/1', {
      status: 200,
      fixture: 'logoAddedProject'
    })
    .get('.logo-button').click()
    .get('.logo-image').should('have.attr', 'src')
    .get('.project-logo > p').should('have.text', 'Makeup 360')
  })
  ...
  it('should display logo if user has already created a logo', () => {
    ...
    .intercept('PUT', 'https://ai-project-planner-be-72e73912044c.herokuapp.com/api/v1/users/1/projects/2', {
      status: 200,
      fixture: 'logoAddedProject'
    })
    .get('[href="/history/2"] > .saved-project > .saved-project-details').click()
    .get('.logo-image').should('have.attr', 'src')
    .should("include", "https://i.imgur.com/0W4rdC1.png")
    .get('.project-logo > p').should('have.text', 'Style Stash')
  })
