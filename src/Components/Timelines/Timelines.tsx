      ...
      <section className='controls'>
        {
          !slideShow
            ? 
            <>
              <button onClick={() => { if (!slideShow) setStepNum(0) }}><img src={doubleleft} alt='timeline control button to first step' /></button>
              ...
              <button onClick={() => { if (!slideShow) setStepNum(steps.length - 1) }}><img src={doubleright} alt='timeline control button to last step' /></button>
              <button onClick={playSlideShow}><img src={play} alt='timeline control button to last step'/></button>
            </>
            : <p>Step {stepNum + 1}</p>
        }
      </section>
