  ...
  return (
    <section className='home'>
      ...
      <section className='popup tutorial-popup'>
        <button className='tutorial-back-btn' onClick={goToPrev} style={{ background: 'none', border: 'none', alignSelf: 'flex-start', position: 'absolute', top: '0' }}>
        ...
        <button onClick={goToNext} className='form-button tutorial-btn'>
          {currentStep !== 3 ? 'CONTINUE' : 'GET STARTED'}
        </button>
        <div className='step-circles'>{stepCircles}</div>
      </section>
    </section>
  );
};
